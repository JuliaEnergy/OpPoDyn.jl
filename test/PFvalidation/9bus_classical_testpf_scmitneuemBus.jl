...
function FaultyLine(;src, dst, R=0.0, X=0.0, B=0.0, fault_R=100, fault_X=100, pos=0.5, vid_fault)
    R_a = R * pos
    X_a = X * pos
    B_a = B * pos
    R_b = R * (1 - pos)
    X_b = X * (1 - pos)
    B_b = B * (1 - pos)
    @named mtkbusfault = LineFaultBus(; faultimp__R_f=fault_R, faultimp__X_f=fault_X)
    @named bus_fault = Bus(mtkbusfault; vidx=vid_fault) 
    @named line_a = Line(piline(; R=R_a, X=X_a, B=B_a), src=src, dst=vid_fault)
    @named line_b = Line(piline(; R=R_b, X=X_b, B=B_b), src=vid_fault, dst=dst)
    return bus_fault, line_a, line_b
end
...
bus_fault, l5f, lf7 = FaultyLine(;src=5, dst=7, R=0.0320, X=0.1610, B=0.3060, pos=0.99, vid_fault=10) #fault_R=195, fault_X=195 #maximal, mehr geht nicht
#ezwas suboptimal, dass man hier schon die Position des Fehlers festlegen muss

# build network
vertexfs = [bus1, bus2, bus3, bus4, bus5, bus6, bus7, bus8, bus9, bus_fault];
edgefs = [l45, l46, l69, l78, l89, t14, t27, t39, lf7, l5f];
...
affect1! = (integrator) -> begin
    if integrator.t == 0.0
        ...
        p.v[10, :faultimp₊R_f] = 0.0000000001#0.003781 #0.0000000001
        p.v[10, :faultimp₊X_f] = 0.0000000001
    else
