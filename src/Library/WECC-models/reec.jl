...
@mtkmodel reec_a begin
    ...
    @parameters begin
        ...
        dP_min, [description="Active power down-ramp limit (pu/s on mbase)"]
        dP_max, [description="Active power up-ramp limit (pu/s on mbase)"]
        Vq1=0.29, [description="q-VDL Table"]
        Vq2=1.33, [description="q-VDL Table"]
        Vq3=1.33, [description="q-VDL Table"]
        Vq4=1.33, [description="q-VDL Table"]
        Iq1=1.25, [description="q-VDL Table"]
        Iq2=0, [description="q-VDL Table"]
        Iq3=0, [description="q-VDL Table"]
        Iq4=0, [description="q-VDL Table"]
        Vp1=0, [description="p-VDL Table"]
        Vp2=1.1, [description="p-VDL Table"]
        Vp3=2, [description="p-VDL Table"]
        Vp4=2, [description="p-VDL Table"]
        Ip1=1.15, [description="p-VDL Table"]
        Ip2=1.24, [description="p-VDL Table"]
        Ip3=1.24, [description="p-VDL Table"]
        Ip4=1.24, [description="p-VDL Table"]
        #I_qfrz, [description="Value at which Iqinj is held for Thld seconds following a voltage dip if Thld > 0"]
        #T_hld=0, [description="Time for which Iqinj is held at Iqfrz after voltage dip returns to zero"]
    end
    @variables begin
        Voltage_dip(t), [description="freeze states if Voltagedip=1"]
        V_tfilt(t), [guess=1, description="Voltage after filter"]
    ...
    @equations begin
        ...
        I_qcmd ~ limiter(I_sum, I_qmin, I_qmax)
        #p-phase current
        P_in ~ (PfFlag * Wg.u + 1-PfFlag) * Pref_in.u
        ΔP ~ P_in - P_refout
        ΔP_lim ~ limiter(ΔP, dP_min, dP_max)
        ...
        I_pmin ~ 0
        I_qmin ~ -I_qmax
        I_pre ~ (1-PqFlag) * (sqrt(I_max) - sqrt(abs(I_qcmd))) + PqFlag * (sqrt(I_max) - sqrt(abs(I_pcmd)))
        I_post ~ ifelse(I_pre<0, 0, sqrt(I_pre))
        I_pmax ~ PqFlag * min(VDL2_out, I_max) + (1-PqFlag) * min(VDL2_out, I_post)
        I_qmax ~ PqFlag * min(VDL1_out, I_post) + (1-PqFlag) * min(VDL1_out, I_max)
        #outputs
        Iqcmd_out.u ~ I_qcmd
        Ipcmd_out.u ~ I_pcmd
    end
