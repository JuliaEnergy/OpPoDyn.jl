...
@mtkmodel StandardModel_pf begin
    ...
    @parameters begin
        ...
        P_T, [guess=1, bounds=(0,Inf), description="Turbine Power input signal in p.u."]
    ...
    @variables begin
        ...
        V′_d(t), [guess=0, description="transient voltage behind transient reactance in d-axis"]
        V′_q(t), [guess=1, description="transient voltage behind transient reactance in q-axis"]
        δ(t), [guess=0, description="rotor angle"]
        ϕ(t), [guess=0, description="angle between d-axis and reference voltage of network"]
        ω(t), [guess=1, description="rotor speed in pu"]
        ...
        # inputs/parameters
        V_f(t), [description="field voltage"]
    ...
    @equations begin
        ...
        V_d ~ V″_d - R_s * I_d + X″_q * I_q * ((1-speedvar) + ω * speedvar) #if neglected: n set to initial speed in this equation, here 1
        V_q ~ V″_q - R_s * I_q - X″_d * I_d * ((1-speedvar) + ω * speedvar)
        V″_d ~ - ψ″_q * (ω* (speedvar_part + speedvar) + (1-(speedvar + speedvar_part)))
        V″_q ~  ψ″_d * (ω*(speedvar_part + speedvar) + (1-(speedvar_part + speedvar)))
        ...
        τ_ag ~ 2 * H
        Dt(ω) ~ (τ_m - τ_e - τ_dkd - τ_dpe) / τ_ag

        #Dt(ϕ) ~ ω_b * (ω - ω_b/(2*π)) #(115)
        #δ ~ ϕ + π/2 #- phiu
        Dt(δ) ~ ω_b * (ω - 1)
        ...
        R_fd * I_fd + 1/ω_b * Dt(ψ_fd) ~ R_fd/X_ad * V_f
        ...
        #Alternative: (102)
        τ_m ~ P_T/ω - xmdm - dpu * ω + addmt #xmdm Torque input signal; addmt additional torque parameter; dpu * ω turbine shaft friction torque
